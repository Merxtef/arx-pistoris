...
TEST_SUITE("fts") {
  TEST_CASE("FtsNullData") {
    ArxFts* h = nullptr;
    ArxReturnCode rc = arx_pistoris_fts_read(nullptr, 0, &h);
  ...
  TEST_CASE("FtsNullOut") {
    uint8_t dummy = 0;
    ArxReturnCode rc = arx_pistoris_fts_read(&dummy, sizeof(dummy), nullptr);
    CHECK(rc == ARX_INVALID_DATA_POINTER);
  }
  ...
  TEST_CASE("FtsReadWriteSmoke") {
    ...
    ArxFts* h = nullptr;
    REQUIRE(arx_pistoris_fts_read(buf.data(), buf.size(), &h) == ARX_OK);
    REQUIRE(h != nullptr);
  ...
  TEST_CASE("FtsWriteCompressesPayloadAndParseAcceptsIt") {
    ...
    REQUIRE(arx_pistoris_fts_read(fixture.data(), fixture.size(), &source) == ARX_OK);
    ...
    CHECK(arx_pistoris_fts_read(out, size, &loaded) == ARX_OK);
