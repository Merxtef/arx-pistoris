...
TEST_SUITE("native fixed string") {
  ...
  TEST_CASE("ClampStrNoop") {
    char arr[8] = "hello";
    canonicalizeFixedString(arr, "test");
    CHECK(arr[6] == '\0');
    CHECK(arr[7] == '\0');
  ...
  TEST_CASE("ClampStrSizes") {
    char s2[2] = {'X', 'Y'};
    canonicalizeFixedString(s2, "test");
    CHECK(s2[1] == '\0');

    char s64[64];
    std::memset(s64, 'Z', sizeof(s64));
    canonicalizeFixedString(s64, "test");
    CHECK(s64[63] == '\0');

    char s256[256];
    std::memset(s256, 'Z', sizeof(s256));
    canonicalizeFixedString(s256, "test");
    CHECK(s256[255] == '\0');
  }
  ...
  TEST_CASE("ClampStrLogUnindexed") {
    LogCapture cap;
    char arr[4] = {'A', 'B', 'C', 'D'};
    canonicalizeFixedString(arr, "FTL: header.name");
  ...
  TEST_CASE("ClampStrLogIndexed") {
    LogCapture cap;
    char arr[4] = {'A', 'B', 'C', 'D'};
    canonicalizeFixedString(arr, "FTL: group.name", 2);
    CHECK(cap.last_level == ARX_LOG_WARN);
  ...
  TEST_CASE("Canonicalization clears bytes after the terminator") {
    char arr[5] = {'a', '\0', 'x', 'y', 'z'};
    canonicalizeFixedString(arr, "test");
    CHECK(fixedStringView(arr).compare("a") == 0);
    CHECK(arr[2] == '\0');
    CHECK(arr[3] == '\0');
    CHECK(arr[4] == '\0');
  }
