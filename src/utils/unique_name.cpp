...
constexpr std::size_t kPortableNameMax = 240;
...
bool portableCharacter(char value) {
  return (value >= 'A' && value <= 'Z') || (value >= 'a' && value <= 'z') || (value >= '0' && value <= '9') ||
         value == '.' || value == '-' || value == '_' || value == '[' || value == ']';
}

std::size_t extensionOffset(std::string_view name) {
  const std::size_t dot = name.find_last_of('.');
  return dot != std::string_view::npos && dot != 0 && dot + 1 < name.size() ? dot : name.size();
}
...
std::string sanitizePortableName(std::string_view requested) {
  std::string result;
...
std::string makeUniqueName(std::string_view requested, const std::unordered_set<std::string>& unavailable) {
  if (!unavailable.contains(std::string(requested))) return std::string(requested);
...
std::string makeUniquePortableName(std::string_view requested, const std::unordered_set<std::string>& unavailable) {
  std::string candidate = isPortableName(requested) ? std::string(requested) : sanitizePortableName(requested);
