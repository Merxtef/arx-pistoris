...
ArxReturnCode readPrefix(fts::Data& data, ReadCursor& cursor) {
  cursor.read(data.header);
  if (!cursor) return ARX_UNEXPECTED_EOF;
  if (data.header.version != kFtsVersion) return ARX_FTS_BAD_VERSION;
  if (!countFits(data.header.count, kFtsMaxHeaderBlocks)) return ARX_FTS_BAD_METADATA_COUNT;

  if (!tryResize(data.unique_headers, data.header.count)) return ARX_BAD_ALLOC;
  return cursor.readArray(data.unique_headers) ? ARX_OK : ARX_UNEXPECTED_EOF;
}
...
ArxReturnCode finishLoad(fts::Data* data, fts::Data& result, ReadCursor& payload) {
  ...
  *data = std::move(result);
  log(ARX_LOG_INFO,
      ...
      data->scene.sizex,
      data->scene.sizez,
      data->scene.num_polys,
      data->textures.size(),
      data->anchors.size(),
      data->portals.size(),
      data->scene.num_rooms);
  return ARX_OK;
}
...
ArxReturnCode loadFts(fts::Data* d, ReadCursor& c) { return loadFts(d, c, c); }

ArxReturnCode loadFts(fts::Data* d, ReadCursor& prefix, ReadCursor& payload) {
  if (!d) return ARX_INVALID_DATA_POINTER;
  fts::Data result;
  ARX_RETURN_IF_ERR(readPrefix(result, prefix));
  return finishLoad(d, result, payload);
}
...
ArxReturnCode validateFts(const fts::Data* d) {
  ...
  if (d->scene.version != kFtsVersion) return ARX_FTS_BAD_VERSION;
  if (!math::finite(d->scene.Mscenepos)) return ARX_FTS_BAD_SCENE_OFFSET;
