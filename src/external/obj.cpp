...
ArxReturnCode exportFtlToObj(const ftl::Data& d, std::string_view obj_stem, std::string& out) {
  ...
  log(ARX_LOG_INFO,
      std::format("OBJ export: {} vertices, {} faces, {} textures",
                  d.vertices.size(),
                  d.faces.size(),
                  d.texture_containers.size()));
  if (!d.groups.empty() || !d.actions.empty() || !d.selections.empty())
    log(ARX_LOG_WARN,
        std::format("OBJ export: {} groups, {} actions, {} selections not exported (no OBJ equivalent)",
                    d.groups.size(),
                    d.actions.size(),
                    d.selections.size()));
  ...
  for (std::size_t i = 0; i < d.texture_containers.size(); ++i) {
    ...
    if (stem.find("__") != std::string_view::npos)
      log(ARX_LOG_WARN,
          std::format("OBJ export: texture[{}] stem '{}' contains \"__\" which is the flag "
...
ArxReturnCode exportFtlToMtl(const ftl::Data& d, std::string& out) {
  ...
  for (const auto& [tex_id, type] : used) {
    if (tex_id == kFtlTextureNone) {
      ...
      if (type & kFaceBitTrans) {
        ...
        if (it != transval_acc.end() && it->second.count > 0) {
          ...
          if (acc.min != acc.max)
            log(ARX_LOG_WARN,
                std::format("MTL export: material '{}' has varying transval [{}, {}], averaging",
                            matName("", type),
                            acc.min,
                            acc.max));
    ...
    if (stem.empty()) {
      log(ARX_LOG_WARN,
          std::format("MTL export: texture [{}] '{}' has no usable stem, skipping map_Kd",
                      tex_id,
...
static ArxReturnCode handleFace(TextCursor& line, std::uint32_t kw_line, const std::vector<ArxVector3>& positions,
  ...
  if (corners.size() == 4)
    ...
    log(ARX_LOG_INFO,
        std::format("OBJ import: {}-gon at line {} triangulated into {} triangles",
                    corners.size(),
                    kw_line,
                    corners.size() - 2));
...
ArxReturnCode importObjToFtl(std::string_view obj, std::string_view mtl, std::string_view obj_filename,
  ...
  if (has_origin && origin_pos_idx >= positions.size()) {
    log(ARX_LOG_WARN,
        std::format(
            "OBJ import: origin index {} out of range ({} positions), ignoring", origin_pos_idx + 1, positions.size()));
    has_origin = false;
  }
  ...
  if (has_origin) {
    ...
    if (!found) {
      if (out->vertices.size() >= kFtlMaxVertices) return ARX_OBJ_TOO_MANY_VERTICES;
      log(ARX_LOG_INFO,
          std::format("OBJ import: origin position {} unreferenced by any face, appended as vertex {}",
                      origin_pos_idx + 1,
                      out->vertices.size()));
  ...
  log(ARX_LOG_INFO,
      std::format("OBJ import done: {} vertices, {} faces, {} textures",
                  out->vertices.size(),
                  out->faces.size(),
  ...
  if (ArxReturnCode rc = validateFtl(out); rc != ARX_OK) {
    log(ARX_LOG_ERROR,
        std::format("OBJ import: constructed data failed validation (rc={}); possible importer bug",
                    static_cast<int>(rc)));
    return rc;
  }
