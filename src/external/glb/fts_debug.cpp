...
ArxReturnCode buildFtsCellsDebugGlb(const fts::Data& fts, std::vector<std::uint8_t>& out,
  ...
  log(ARX_LOG_INFO, std::format("FTS debug cells GLB export: {} cells, {} polygons", emitted_cells, emitted_polys));
...
ArxReturnCode buildFtsRoomsDebugGlb(const fts::Data& fts, std::vector<std::uint8_t>& out) {
  ...
  log(ARX_LOG_INFO,
      std::format("FTS debug rooms GLB export: {} rooms, {} polygons, {} portals",
                  emitted_rooms,
                  emitted_polys,
                  emitted_portals));
  return builder.write(out);
