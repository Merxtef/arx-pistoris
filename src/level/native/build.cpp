...
void logBuildWarnings(const NativeBuildWarnings& warnings) {
  const bool has_repairs =
      ...
      warnings.repaired_portal_names != 0 || warnings.repaired_light_names != 0 || warnings.repaired_fog_names != 0 ||
      warnings.repaired_zone_names != 0 || warnings.repaired_path_names != 0 ||
      warnings.room_distance_no_positive_real_pairs;
  if (has_repairs)
    logLazy(ARX_LOG_WARN, [&] {
      ...
      if (warnings.repaired_portal_names != 0)
        warning += std::format(" {} portal name(s) repaired;", warnings.repaired_portal_names);
      if (warnings.repaired_light_names != 0)
        warning += std::format(" {} light name(s) repaired;", warnings.repaired_light_names);
      if (warnings.repaired_fog_names != 0)
        warning += std::format(" {} fog name(s) repaired;", warnings.repaired_fog_names);
      if (warnings.repaired_zone_names != 0)
        warning += std::format(" {} zone name(s) repaired;", warnings.repaired_zone_names);
      if (warnings.repaired_path_names != 0)
        warning += std::format(" {} path name(s) repaired;", warnings.repaired_path_names);
  ...
  if (warnings.outside_geometry_anchors != 0)
    log(ARX_LOG_WARN,
        "Native -> Level: {} anchor(s) outside referenced geometry bounds retained",
...
ArxReturnCode buildLevel(const NativeLevelSource& source, LevelModules& out, LevelValidationState* out_validation,
  ...
  warnings.repaired_light_names += lights::repairLightNames(tmp.lighting.lights);
  warnings.repaired_fog_names += scene::repairFogNames(tmp.scene.fogs);
  warnings.repaired_zone_names += scene::repairZoneNames(tmp.scene.zones);
  if (source.llf && !use_llf_colors) {
    log(ARX_LOG_WARN,
        "FTS/LLF color count mismatch: expected {}, got {}; ignored all baked colors and preserved {} "
        "light(s)",
        expected_colors,
