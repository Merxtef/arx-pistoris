...
struct ComponentTopology {
  explicit ComponentTopology(std::size_t triangle_count)
      : components(triangle_count), component_count(triangle_count) {}
...
struct ComponentAnalysis {
  ComponentAnalysis(ComponentTopology&& topology, std::size_t triangle_count)
      : components(std::move(topology.components)),
        area_by_component(triangle_count, 0.0),
        triangles_by_component(triangle_count, 0) {}

  DisjointSet components;
  std::vector<double> area_by_component;
  std::vector<std::size_t> triangles_by_component;
...
double triangleArea(const NavSurface& surface, const NavSurfaceTriangle& triangle) {
  ...
  return math::length(math::cross(ab, ac)) * 0.5;
}
...
ComponentTopology buildComponentTopology(const NavSurface& surface) {
  ComponentTopology topology(surface.triangles.size());
  std::unordered_map<std::uint64_t, std::size_t> owner_by_edge;
  owner_by_edge.reserve(surface.triangles.size() * 3U);
  for (std::size_t triangle_index = 0; triangle_index < surface.triangles.size(); ++triangle_index) {
    ...
    for (std::size_t edge = 0; edge < vertices.size(); ++edge) {
      ...
      if (!inserted && topology.components.unite(triangle_index, owner->second)) --topology.component_count;
    }
  }
  return topology;
}
...
ComponentAnalysis analyzeComponents(const NavSurface& surface) {
  ComponentAnalysis analysis(buildComponentTopology(surface), surface.triangles.size());
  for (std::size_t triangle_index = 0; triangle_index < surface.triangles.size(); ++triangle_index) {
    const std::size_t component = analysis.components.find(triangle_index);
...
std::size_t surfaceComponentCount(const NavSurface& surface) { return buildComponentTopology(surface).component_count; }

Error planSurfaceComponentPrune(NavSurfaceComponentPrunePlan& out, const NavSurface& surface,
                                const NavSurfacePruneOptions& options, NavSurfacePruneDiagnostics* diagnostics) {
  ...
  if (error != Error::kNone) return error;
  out.replacement = std::move(kept_surface);
  out.removed_components = removed_components;
  out.removed_triangles = removed_triangles;
  out.removed_area = removed_area;
  return Error::kNone;
}
...
Error pruneSurfaceComponents(NavSurface& surface, const NavSurfacePruneOptions& options,
  ...
  NavSurfaceComponentPrunePlan plan;
  const Error error = planSurfaceComponentPrune(plan, surface, options, diagnostics);
  if (error == Error::kNone) applySurfaceComponentPrune(surface, std::move(plan));
  return error;
}

void applySurfaceComponentPrune(NavSurface& surface, NavSurfaceComponentPrunePlan&& plan) noexcept {
  if (!plan.replacement) return;
  surface = std::move(*plan.replacement);
  log(ARX_LOG_WARN,
      "Navigation surface pruning removed {} component(s), {} triangle(s), area {:.3f}",
      plan.removed_components,
      plan.removed_triangles,
      plan.removed_area);
}
