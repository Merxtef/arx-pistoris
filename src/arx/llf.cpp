...
ArxReturnCode loadLlf(llf::Data* data, ReadCursor& cursor) {
  ...
  log(ARX_LOG_INFO, std::format("LLF loaded: {} lights, {} colors", tmp.lights.size(), tmp.colors.size()));
  *data = std::move(tmp);
  return ARX_OK;
}
...
ArxReturnCode saveLlf(const llf::Data* data, std::string_view signer, WriteCursor& cursor) {
  ...
  log(ARX_LOG_INFO, std::format("LLF saving: {} lights, {} colors", data->lights.size(), data->colors.size()));
...
ArxReturnCode validateLlf(const llf::Data* data) {
  ...
  for (const llf::Light& light : data->lights) {
    if (!finite(light.position)) return ARX_LLF_BAD_LIGHT_POSITION;
    if (!unitColor(light.color)) return ARX_LLF_BAD_LIGHT_COLOR;
    if (!finite(light.fallstart) || light.fallstart < 0.0f || !finite(light.fallend) || light.fallend < light.fallstart)
      return ARX_LLF_BAD_LIGHT_FALLOFF;
    if (!finite(light.intensity) || light.intensity < 0.0f) return ARX_LLF_BAD_LIGHT_INTENSITY;
