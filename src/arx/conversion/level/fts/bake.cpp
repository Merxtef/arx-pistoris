...
// Each bit identifies one original triangle edge incident to the source corner
constexpr std::array<std::uint8_t, 3> kSourceCornerEdgeMasks = {0b101U, 0b011U, 0b110U};

using BakedPolygonColors = std::array<ArxColor3, 4>;
...
std::vector<BakedCorner> clipHalfPlane(const std::vector<BakedCorner>& input, bool x_axis, float boundary,
                                       bool keep_greater) {
  std::vector<BakedCorner> output;
...
std::vector<BakedCorner> clipToCell(std::vector<BakedCorner> polygon, std::int32_t cell_x, std::int32_t cell_z) {
  ...
  polygon = clipHalfPlane(polygon, true, min_x, true);
  polygon = clipHalfPlane(polygon, true, max_x, false);
  polygon = clipHalfPlane(polygon, false, min_z, true);
  polygon = clipHalfPlane(polygon, false, max_z, false);
  polygon.erase(std::unique(polygon.begin(),
                            polygon.end(),
                            [](const BakedCorner& first, const BakedCorner& second) {
                              return first.position == second.position;
                            }),
                polygon.end());
  if (polygon.size() > 1 && polygon.front().position == polygon.back().position) polygon.pop_back();
  return polygon;
}
...
ArxReturnCode copyTexturePath(const std::string& path, fts::Texture& out) {
  if (path.empty() || path.size() >= sizeof(out.fic)) return ARX_FTS_BAD_TEXTURE_PATH;
  std::memcpy(out.fic, path.c_str(), path.size() + 1);
  return ARX_OK;
}
...
void logRoomDistanceBakeWarnings(const LevelModules& level) {
  ...
  if (!rooms::hasCompleteRoomDistances(level.rooms.distances, level.rooms.definitions.size())) {
    if (pair_count != 0) {
      log(ARX_LOG_WARN,
          std::format("Level native bake: room-distance data missing or incomplete; writing default -1 "
  ...
  if (missing_connections != 0) {
    log(ARX_LOG_WARN,
        std::format("Level native bake: {} room-distance connection(s) missing; writing -1 with zero endpoints",
                    missing_connections));
  }
...
ArxReturnCode bakeFts(const LevelModules& level, NativeTextureResources& textures, bool reconstruct_quads,
  ...
  if (level.geometry.textures.size() > kFtsMaxTextures) return ARX_FTS_BAD_TEXTURE_COUNT;
  if (textures.families.size() != level.geometry.textures.size()) return ARX_FTS_BAD_TEXTURE_COUNT;
  const std::size_t native_room_count = level.rooms.definitions.size() + 1U;
  ...
  fts.scene.Mscenepos = {};
  fts.scene.playerpos = level.scene.player_spawn.position;
  ...
  std::vector<std::vector<BakedPolygonColors>> cell_colors(fts.cells.size());
  ...
  for (std::size_t face_index = 0; face_index < level.geometry.faces.size(); ++face_index) {
    ...
    std::vector<BakedCorner> triangle;
    triangle.reserve(3);
    ...
    for (std::int32_t cell_z = min_cell_z; cell_z <= max_cell_z; ++cell_z) {
      for (std::int32_t cell_x = min_cell_x; cell_x <= max_cell_x; ++cell_x) {
        const std::vector<BakedCorner> clipped = clipToCell(triangle, cell_x, cell_z);
  ...
  for (std::size_t cell_idx = 0; cell_idx < fts.cells.size(); ++cell_idx) {
    std::vector<BakedPolygon> polygons = fts_bake::packCellPolygons(cell_triangles[cell_idx], reconstruct_quads);
    ...
    for (const BakedPolygon& source : polygons) {
      ...
      cell_colors[cell_idx].push_back(
          {source.corners[0].color, source.corners[1].color, source.corners[2].color, source.corners[3].color});
  ...
  if (rc != ARX_OK) return rc;
  baked_colors.clear();
  baked_colors.reserve(polygon_count * 4U);
  for (std::size_t cell_idx = 0; cell_idx < fts.cells.size(); ++cell_idx) {
    const fts::Cell& cell = fts.cells[cell_idx];
    for (std::size_t poly_idx = 0; poly_idx < cell.polygons.size(); ++poly_idx) {
      const std::size_t count = (cell.polygons[poly_idx].type & kFaceBitQuad) != 0 ? 4U : 3U;
      for (std::size_t corner = 0; corner < count; ++corner)
        baked_colors.push_back(cell_colors[cell_idx][poly_idx][corner]);
    }
  }
  out = std::move(fts);
