...
TextureIndex addTexture(LevelModules& out, std::map<std::int32_t, TextureIndex>& by_id, std::int32_t texture_id,
                        std::string texture) {
  ...
  texture = paths::textureFromGame(texture);
  TextureIndex index = static_cast<TextureIndex>(out.geometry.textures.size());
  out.geometry.textures.push_back(std::move(texture));
  by_id.emplace(texture_id, index);
  return index;
}

void addTriangle(LevelModules& out, const fts::Poly& poly, int a, int b, int c,
                 std::array<VertexIndex, 4>& polygon_vertices, TextureIndex texture, FaceType flags, std::uint32_t room,
                 const ArxColor3* source_colors, NativeBuildWarnings& warnings) {
  ...
  for (std::size_t i = 0; i < source_indices.size(); ++i) {
    ...
    if (index == kInvalidVertexIndex) index = geometry::addVertex(out.geometry, positions[i]);
  ...
  for (std::size_t i = 0; i < face.corners.size(); ++i) {
    ...
    face.corners[i] = {vertex_indices[i], normal, poly.v[source_index].stu, poly.v[source_index].stv};
  }
...
void importRoomDistances(const fts::Data& fts, LevelModules& out, NativeBuildWarnings& warnings) {
  ...
  rooms::initializeRoomDistances(out.rooms.distances, out.rooms.definitions.size());
  const std::size_t native_room_count = static_cast<std::size_t>(fts.scene.num_rooms) + 1U;
...
ArxReturnCode buildFtsModules(const fts::Data& fts, std::span<const ArxColor3> colors, LevelModules& out,
  ...
  for (std::int32_t id : texture_ids) addTexture(out, textures, id, fts.textures.at(id).fic);
  ...
  for (std::int32_t z = 0; z < fts.scene.sizez; ++z) {
    for (std::int32_t x = 0; x < fts.scene.sizex; ++x) {
      ...
      for (const fts::Poly& poly : cell.polygons) {
        ...
        if (poly.tex > 0) texture = addTexture(out, textures, poly.tex, fts.textures.at(poly.tex).fic);
        ...
        if ((poly.type & kFaceBitQuad) != 0)
          addTriangle(out, poly, 3, 2, 1, polygon_vertices, texture, flags, room, source, warnings);
        color_index += corners;
      }
    }
  }
