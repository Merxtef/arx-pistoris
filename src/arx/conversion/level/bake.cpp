...
void logTextureShardWarnings(const NativeTextureResources& textures) {
  for (const NativeTextureFamily& family : textures.families) {
    ...
    log(ARX_LOG_WARN,
        std::format("Level native bake: texture resource '{}' was sharded as {} to stay within {} corners per room "
...
ArxReturnCode bakeValidatedNativeLevelBundle(const LevelModules& level, const Level::NativeBakeOptions& options,
  ...
  ArxReturnCode rc = projectNativeTextures(level.geometry.textures, options, textures, warnings);
  ...
  rc = bakeLlf(level.lighting, baked_colors, tmp.llf);
  if (rc != ARX_OK) return rc;
  ...
  if (warnings.discarded_fragments != 0)
    log(ARX_LOG_WARN,
        std::format("Level native bake: {} clipped polygon fragment(s) with non-finite or minimum area discarded",
                    warnings.discarded_fragments));
  if (warnings.fully_discarded_faces != 0)
    log(ARX_LOG_WARN,
        std::format("Level native bake: {} source face(s) produced no output polygons after clipping",
                    warnings.fully_discarded_faces));
  if (warnings.rescaled_texture_images != 0)
    log(ARX_LOG_WARN,
        std::format("Level native bake: rescaled {} non-power-of-two texture image(s) to power-of-two dimensions for "
  ...
  if (options.reconstruct_quads) {
    ...
    log(ARX_LOG_DEBUG,
        std::format("Level native bake: reconstructed {} FTS quad(s): {} clipped-fragment, {} cross-face; {} "
                    "triangle(s) remain",
                    reconstructed_quads,
    ...
    log(ARX_LOG_DEBUG,
        std::format("Level native bake: FTS quad reconstruction disabled; emitted {} triangle(s)",
                    statistics.triangles));
  }
  log(ARX_LOG_INFO,
      std::format("Level native bake: {} FTS polygon(s), {} LLF color(s), DLF scene '{}'",
