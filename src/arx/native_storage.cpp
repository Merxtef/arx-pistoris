...
ArxReturnCode loadFtsStorage(fts::Data* data, std::span<const std::uint8_t> stored) {
  ...
  if (pkware::looksLikeDcl(payload)) {
    ...
    if (compressed_rc == ARX_OK) {
      std::vector<std::uint8_t> raw = join(stored.first(prefix_size), decoded);
      fts::Header raw_header = header;
      raw_header.uncompressedsize = 0;
      std::memcpy(raw.data(), &raw_header, sizeof(raw_header));

      fts::Data candidate;
      compressed_rc = parseFts(&candidate, raw);
      if (compressed_rc == ARX_OK) {
        candidate.header.uncompressedsize = header.uncompressedsize;
...
ArxReturnCode loadDlfStorage(dlf::Data* data, std::optional<llf::Data>* embedded_lighting,
  ...
  if (compressed_rc == ARX_BAD_ALLOC) return compressed_rc;
  if (compressed_rc == ARX_OK) {
    std::vector<std::uint8_t> raw = join(stored.first(kDlfRawPrefixSize), decoded);
    dlf::Data candidate;
    std::optional<llf::Data> candidate_lighting;
    compressed_rc = parseDlf(&candidate, embedded_lighting ? &candidate_lighting : nullptr, raw);
    if (compressed_rc == ARX_OK) {
      *data = std::move(candidate);
