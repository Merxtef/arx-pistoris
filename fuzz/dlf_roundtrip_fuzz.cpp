...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, std::size_t size) {
  ...
  if (arx_pistoris_dlf_parse(data, size, &raw_source, &raw_embedded_lighting) != ARX_OK) return 0;
  ...
  if (arx_pistoris_dlf_write(source.get(), &options, 0, &bytes.value, &bytes.byte_count) != ARX_OK) {
    return 0;
  }
  ...
  const ArxReturnCode read_rc =
      arx_pistoris_dlf_parse(bytes.get(), bytes.size(), &raw_roundtrip, &raw_roundtrip_lighting);
