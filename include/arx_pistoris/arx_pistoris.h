...
#include "arx_pistoris/api.h"             // IWYU pragma: export
#include "arx_pistoris/buffer.h"          // IWYU pragma: export
...
// null opaque input handles return ARX_INVALID_HANDLE
// null required data and output pointers return ARX_INVALID_DATA_POINTER
...
ARX_API const char* arx_pistoris_get_layout_hash(void) ARX_NOEXCEPT;

ARX_API void arx_pistoris_set_log_callback(ArxLogFn fn, void* userdata) ARX_NOEXCEPT;

// reads raw or whole-file compressed FTL
ARX_API ArxReturnCode arx_pistoris_ftl_parse(const uint8_t* data, size_t size, ArxFtlHandle* out) ARX_NOEXCEPT;
ARX_API void arx_pistoris_ftl_free(ArxFtlHandle h) ARX_NOEXCEPT;

// static geometry only; groups/actions/selections dropped
ARX_API ArxReturnCode arx_pistoris_ftl_to_obj(ArxFtlHandle h, const char* obj_stem, char** out) ARX_NOEXCEPT;
ARX_API ArxReturnCode arx_pistoris_ftl_to_mtl(ArxFtlHandle h, char** out) ARX_NOEXCEPT;
// pretty != 0: 2-space indent
ARX_API ArxReturnCode arx_pistoris_ftl_to_json(ArxFtlHandle h, uint32_t pretty, char** out) ARX_NOEXCEPT;
ARX_API ArxReturnCode arx_pistoris_ftl_from_json(const uint8_t* data, size_t size, ArxFtlHandle* out) ARX_NOEXCEPT;
ARX_API ArxReturnCode arx_pistoris_tea_to_json(ArxTeaHandle h, uint32_t pretty, char** out) ARX_NOEXCEPT;
ARX_API ArxReturnCode arx_pistoris_tea_from_json(const uint8_t* data, size_t size, ArxTeaHandle* out) ARX_NOEXCEPT;

// dead fields are zero-padded; compress == 0 writes raw storage, every other value enables compression
ARX_API ArxReturnCode arx_pistoris_ftl_write(ArxFtlHandle h, uint32_t compress, uint8_t** out,
                                             size_t* out_size) ARX_NOEXCEPT;

ARX_API ArxReturnCode arx_pistoris_tea_parse(const uint8_t* data, size_t size, ArxTeaHandle* out) ARX_NOEXCEPT;
ARX_API void arx_pistoris_tea_free(ArxTeaHandle h) ARX_NOEXCEPT;

// dead fields zero-padded; audio dropped, sample_size = 0
ARX_API ArxReturnCode arx_pistoris_tea_write(ArxTeaHandle h, uint8_t** out, size_t* out_size) ARX_NOEXCEPT;

// pointer to anim_name (null-terminated, lifetime tied to h); NULL if h is null; do not free
ARX_API const char* arx_pistoris_tea_name(ArxTeaHandle h) ARX_NOEXCEPT;
...
ARX_API ArxReturnCode arx_pistoris_from_glb(const uint8_t* data, size_t size, const char* glb_filename,
                                            ArxFtlHandle* out_ftl, ArxTeaHandle** out_teas,
                                            size_t* out_tea_count) ARX_NOEXCEPT;
ARX_API void arx_pistoris_free_tea_array(ArxTeaHandle* teas, size_t count) ARX_NOEXCEPT;

ARX_API ArxReturnCode arx_pistoris_ftl_validate(ArxFtlHandle ftl_h) ARX_NOEXCEPT;
ARX_API ArxReturnCode arx_pistoris_tea_validate(ArxTeaHandle tea_h) ARX_NOEXCEPT;
...
ARX_API ArxReturnCode arx_pistoris_ftl_overwrite_texture_paths(ArxFtlHandle ftl_h, const char* path) ARX_NOEXCEPT;
...
ARX_API ArxReturnCode arx_pistoris_ftl_apply_xform(ArxFtlHandle ftl_h, float rx, float ry, float rz, float sx, float sy,
                                                   float sz, float tx, float ty, float tz) ARX_NOEXCEPT;
...
ARX_API ArxReturnCode arx_pistoris_ftl_snap_bone_origins_to_reference(ArxFtlHandle target,
                                                                      ArxFtlHandle reference) ARX_NOEXCEPT;
ARX_API ArxReturnCode arx_pistoris_ftl_snap_action_points_to_reference(ArxFtlHandle target,
                                                                       ArxFtlHandle reference) ARX_NOEXCEPT;
...
ARX_API ArxReturnCode arx_pistoris_obj_parse(const uint8_t* obj_data, size_t obj_size, const uint8_t* mtl_data,
                                             size_t mtl_size, const char* obj_filename, ArxFtlHandle* out) ARX_NOEXCEPT;
