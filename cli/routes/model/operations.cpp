...
bool renameSelections(pistoris::Ftl& ftl, std::string_view csv) {
  ...
  while (pos <= csv.size()) {
    ...
    if (idx >= ftl.selections.size()) {
      cli::diagnostic(cli::DiagnosticCode::kModelModuleInvalid,
                      "--rename-selections: got more names than selections (%zu)",
                      ftl.selections.size());
      return false;
    }
    ...
    if (!name.empty()) {
      ...
      cli::log(ARX_LOG_INFO, "renaming selection: %s -> %.*s", sel.name, static_cast<int>(name.size()), name.data());
      std::memset(sel.name, 0, sizeof(sel.name));
      std::size_t n = name.size();
      if (n >= sizeof(sel.name)) {
        n = sizeof(sel.name) - 1;
        cli::log(ARX_LOG_WARN,
                 "--rename-selections: selection name '%.*s' truncated to %zu chars",
...
bool referenceOperationRequested(const ModelOptions& options) {
  return options.autosize_to_reference || options.bone_origin_reference_mode != BoneOriginReferenceMode::kNone ||
         options.snap_action_points || options.copy_reference_affiliations;
}

bool requireReference(const Context& ctx) {
  if (ctx.has_reference) return true;
  cli::diagnostic(cli::DiagnosticCode::kModelModuleInvalid, "reference operation requires --ftl-reference");
  return false;
}
...
pistoris::ArxVector3 rotateAroundAxis(const pistoris::ArxVector3& v, const pistoris::ArxVector3& axis, float c,
                                      float s) {
  return v * c + pistoris::math::cross(axis, v) * s + axis * (pistoris::math::dotf(axis, v) * (1.0f - c));
}

pistoris::ArxVector3 rotateBetween(const pistoris::ArxVector3& v, const pistoris::ArxVector3& from,
                                   const pistoris::ArxVector3& to) {
  pistoris::ArxVector3 f = pistoris::math::normalizeFiniteOr(from, {1.0f, 0.0f, 0.0f}, 1e-8);
  pistoris::ArxVector3 t = pistoris::math::normalizeFiniteOr(to, f, 1e-8);
  float c = std::clamp(pistoris::math::dotf(f, t), -1.0f, 1.0f);
  if (c > 0.9999f) return v;

  pistoris::ArxVector3 axis = pistoris::math::cross(f, t);
  float axis_len = pistoris::math::lengthf(axis);
  if (axis_len <= 1e-6f) {
    pistoris::ArxVector3 fallback =
        std::abs(f.x) < 0.9f ? pistoris::ArxVector3{1.0f, 0.0f, 0.0f} : pistoris::ArxVector3{0.0f, 1.0f, 0.0f};
    axis = pistoris::math::normalizeFiniteOr(pistoris::math::cross(f, fallback), {0.0f, 0.0f, 1.0f}, 1e-8);
...
void warnGroupMismatch(const pistoris::Ftl& target, const pistoris::Ftl& ref) {
  if (target.groups.size() != ref.groups.size()) {
    cli::log(ARX_LOG_WARN,
             "reference: group count mismatch target=%zu reference=%zu",
             target.groups.size(),
             ref.groups.size());
    return;
  }
  for (std::size_t i = 0; i < target.groups.size(); ++i) {
    ...
    if (tn != rn) {
      cli::log(ARX_LOG_WARN,
               "reference: group %zu name mismatch target='%s' reference='%s'",
               i,
  ...
  if (target.extras.parent_bone.size() == ref.extras.parent_bone.size()) {
    for (std::size_t i = 0; i < target.extras.parent_bone.size(); ++i) {
      if (target.extras.parent_bone[i] != ref.extras.parent_bone[i]) {
        cli::log(ARX_LOG_WARN,
                 "reference: group %zu parent mismatch target=%d reference=%d",
                 i,
                 target.extras.parent_bone[i],
                 ref.extras.parent_bone[i]);
      }
    }
  }
}
...
std::vector<Landmark> collectReferenceLandmarks(const pistoris::Ftl& target, const pistoris::Ftl& ref) {
  ...
  if (validVertex(target, target.header.origin) && validVertex(ref, ref.header.origin)) {
    out.push_back({"header.origin",
                   target.vertices[target.header.origin].position,
                   ref.vertices[ref.header.origin].position,
                   1.0f});
  }
  ...
  for (std::size_t i = 0; i < ng; ++i) {
    ...
    out.push_back({std::string{"group:"} + target.groups[i].name,
                   target.vertices[target.groups[i].origin].position,
                   ref.vertices[ref.groups[i].origin].position,
                   1.0f});
  }
  ...
  for (const auto& ref_action : ref.actions) {
    ...
    out.push_back({std::string{"action:"} + ref_action.name,
                   target.vertices[target_action.vertex_idx].position,
                   ref.vertices[ref_action.vertex_idx].position,
                   actionWeight(ref_action.name)});
  }
  return out;
}
...
bool autosizeToReference(pistoris::Ftl& target, const pistoris::Ftl& ref) {
  ...
  if (lm.empty()) {
    cli::diagnostic(cli::DiagnosticCode::kModelModuleInvalid, "--autosize-to-reference: no usable landmarks");
    return false;
  }
  ...
  for (const auto& l : lm) {
    wsum += l.weight;
    ct = ct + l.target * l.weight;
    cr = cr + l.reference * l.weight;
  }
  ct = ct / wsum;
  cr = cr / wsum;
  ...
  for (const auto& l : lm) {
    pistoris::ArxVector3 a = l.target - ct;
    pistoris::ArxVector3 b = l.reference - cr;
    num += l.weight * pistoris::math::dotf(a, b);
    den += l.weight * pistoris::math::dotf(a, a);
  }

  float scale = den > 1e-8f ? num / den : 1.0f;
  if (scale <= 0.0f || !std::isfinite(scale)) {
    cli::diagnostic(
        cli::DiagnosticCode::kModelModuleInvalid, "--autosize-to-reference: invalid fitted scale %f", scale);
    return false;
  }
  pistoris::ArxVector3 offset = cr - ct * scale;

  for (auto& v : target.vertices) v.position = v.position * scale + offset;

  float sum_sq = 0.0f;
  float max_err = -1.0f;
  std::string worst;
  for (const auto& l : lm) {
    pistoris::ArxVector3 fitted = l.target * scale + offset;
    float e = pistoris::math::lengthf(fitted - l.reference);
    sum_sq += l.weight * e * e;
    if (e > max_err) {
      max_err = e;
      worst = l.name;
  ...
  cli::log(ARX_LOG_INFO,
           "autosize-to-reference: landmarks=%zu scale=%f offset=(%f,%f,%f) rms=%f max=%f (%s)",
           lm.size(),
           scale,
           offset.x,
           offset.y,
...
bool deltaDeformBoneOrigins(pistoris::Ftl& target, const pistoris::Ftl& ref) {
  ...
  for (std::size_t i = 0; i < target.groups.size(); ++i) {
    if (!validVertex(target, target.groups[i].origin) || !validVertex(ref, ref.groups[i].origin)) {
      cli::diagnostic(cli::DiagnosticCode::kModelModuleInvalid,
                      "--snap-bone-origins-to-reference delta-deform: group %zu has invalid origin",
                      i);
      return false;
    }
    ...
    deltas[i] = ref_pos - target_pos;
    float d = pistoris::math::lengthf(ref_pos - target_pos);
  ...
  for (std::size_t vi = 0; vi < target.vertices.size(); ++vi) {
    ...
    target.vertices[vi].position = target.vertices[vi].position + deltas[owner];
    ++moved;
...
pistoris::ArxVector3 applyEdgeTransform(const EdgeTransform& edge, const pistoris::ArxVector3& point) {
  pistoris::ArxVector3 local = point - edge.parent_current;
  pistoris::ArxVector3 src_dir = edge.source_axis / edge.source_len;
  pistoris::ArxVector3 dst_dir = edge.target_axis / edge.target_len;
  float parallel_len = pistoris::math::dotf(local, src_dir);
  pistoris::ArxVector3 parallel = dst_dir * (parallel_len * edge.axial_scale);
  pistoris::ArxVector3 perp = local - src_dir * parallel_len;
  pistoris::ArxVector3 rotated_perp = rotateBetween(perp, edge.source_axis, edge.target_axis);
  return edge.parent_reference + parallel + rotated_perp * edge.width_scale;
}
...
pistoris::ArxVector3 applyEdgeTransformRigid(const EdgeTransform& edge, const pistoris::ArxVector3& point) {
  ...
  pistoris::ArxVector3 dst_dir = edge.target_axis / edge.target_len;
  float parallel_len = pistoris::math::dotf(local, src_dir);
  pistoris::ArxVector3 parallel = dst_dir * parallel_len;
  pistoris::ArxVector3 perp = local - src_dir * parallel_len;
  pistoris::ArxVector3 rotated_perp = rotateBetween(perp, edge.source_axis, edge.target_axis);
  return edge.parent_reference + parallel + rotated_perp;
}
...
float medianDirectChildLengthRatio(const pistoris::Ftl& target, const pistoris::Ftl& ref, std::size_t root,
  ...
  for (std::size_t child : children[root]) {
    if (!validVertex(target, target.groups[root].origin) || !validVertex(target, target.groups[child].origin) ||
      ...
      continue;
    float src = pistoris::math::lengthf(target.vertices[target.groups[child].origin].position -
                                        target.vertices[target.groups[root].origin].position);
    float dst = pistoris::math::lengthf(ref.vertices[ref.groups[child].origin].position -
                                        ref.vertices[ref.groups[root].origin].position);
...
bool hierarchyDeformBoneOrigins(pistoris::Ftl& target, const pistoris::Ftl& ref, std::size_t step_limit) {
  ...
  if (target.groups.empty()) {
    cli::log(ARX_LOG_INFO, "snap-bone-origins-to-reference hierarchy-deform: no bone groups");
    return true;
  }
  if (!sameTopology(target, ref)) {
    cli::diagnostic(cli::DiagnosticCode::kModelModuleInvalid,
                    "--snap-bone-origins-to-reference hierarchy-deform: parent topology mismatch");
    return false;
  }
  warnGroupMismatch(target, ref);
  ...
  for (std::size_t i = 0; i < target.groups.size(); ++i)
    if (!validVertex(target, target.groups[i].origin) || !validVertex(ref, ref.groups[i].origin)) {
      cli::diagnostic(cli::DiagnosticCode::kModelModuleInvalid,
                      "--snap-bone-origins-to-reference hierarchy-deform: group %zu has invalid origin",
                      i);
      return false;
    }
  ...
  if (roots.size() != 1) {
    cli::diagnostic(cli::DiagnosticCode::kModelModuleInvalid,
                    "--snap-bone-origins-to-reference hierarchy-deform: expected exactly one root, got %zu",
                    roots.size());
    return false;
  }
  ...
  pistoris::ArxVector3 root_delta =
      ref.vertices[ref.groups[root].origin].position - target.vertices[target.groups[root].origin].position;
  ...
  for (std::size_t vi = 0; vi < target.vertices.size(); ++vi) {
    ...
    target.vertices[vi].position = target.vertices[vi].position + root_delta;
  ...
  auto move_subtree_with_delta = [&](std::size_t child_root, const pistoris::ArxVector3& delta) {
    for (std::size_t vi = 0; vi < target.vertices.size(); ++vi) {
      ...
      if (!should_move) continue;
      target.vertices[vi].position = target.vertices[vi].position + delta;
      if (synthetic.contains(static_cast<std::int32_t>(vi)))
        moved_synthetic.insert(vi);
      else
        moved_vertices.insert(vi);
  ...
  if (step_limit == 0) {
    cli::log(ARX_LOG_INFO, "snap-bone-origins-to-reference hierarchy-deform: stopped after debug step limit 0");
    print_summary();
    return true;
  }
  ...
  for (std::size_t parent = 0; parent < target.groups.size(); ++parent) {
    ...
    for (std::size_t child : children[parent]) {
      if (processed_steps >= step_limit) {
        cli::log(ARX_LOG_INFO,
                 "snap-bone-origins-to-reference hierarchy-deform: stopped after debug step limit %zu",
                 step_limit);
        print_summary();
        return true;
      }
      ...
      edge.source_axis = edge.child_current - edge.parent_current;
      edge.target_axis = ref.vertices[ref.groups[child].origin].position - edge.parent_reference;
      edge.source_len = pistoris::math::lengthf(edge.source_axis);
      edge.target_len = pistoris::math::lengthf(edge.target_axis);
      max_presnap = std::max(
          max_presnap, pistoris::math::lengthf(edge.child_current - ref.vertices[ref.groups[child].origin].position));
      if (edge.target_len <= kBoneEdgeEpsilon) {
        cli::log(ARX_LOG_WARN,
                 ...
                 target.groups[parent].name,
                 target.groups[child].name);
        move_subtree_with_delta(child, ref.vertices[ref.groups[child].origin].position - edge.child_current);
    ...
    for (std::size_t vi = 0; vi < target.vertices.size(); ++vi) {
      ...
      float parent_w =
          2.0f /
          std::max(1e-4f,
                   pistoris::math::lengthSquaredf(original - target.vertices[target.groups[parent].origin].position));
      pistoris::ArxVector3 accum = original * parent_w;
      float wsum = parent_w;
      for (const auto& edge : edges) {
        float w = 1.0f / std::max(1e-4f, pistoris::math::lengthSquaredf(original - edge.child_current));
        accum = accum + applyEdgeTransform(edge, original) * w;
        wsum += w;
      }
      target.vertices[vi].position = accum / wsum;
      moved_vertices.insert(vi);
    }
  }
...
bool applyModules(Context& ctx, const ModelOptions& options, const SharedConversionOptions& conversion) {
  if (referenceOperationRequested(options)) {
  ...
  if (conversion.has_xform) {
    ...
    if (rc != ARX_OK) {
      cli::diagnostic(cli::DiagnosticCode::kModelModuleFailed,
                      "FTL transform failed: %s (code %d)",
  ...
  if (options.overwrite_texture) {
    ArxReturnCode rc = pistoris::overwriteTexturePaths(ctx.ftl, options.overwrite_texture);
    if (rc != ARX_OK) {
      cli::diagnostic(cli::DiagnosticCode::kModelModuleFailed,
                      "Texture overwrite failed: %s (code %d)",
                      pistoris::errorString(rc),
                      static_cast<int>(rc));
      return false;
    }
  ...
  if (options.rename_selections) {
    if (!renameSelections(ctx.ftl, options.rename_selections)) return false;
  }

  if (options.autosize_to_reference) {
    if (!autosizeToReference(ctx.ftl, ctx.reference_ftl)) return false;
  }

  switch (options.bone_origin_reference_mode) {
    ...
    case BoneOriginReferenceMode::kSnapOrigins: {
      ...
      if (rc != ARX_OK) {
        cli::diagnostic(cli::DiagnosticCode::kModelModuleFailed,
                        "--snap-bone-origins-to-reference failed: %s (code %d)",
                        pistoris::errorString(rc),
                        static_cast<int>(rc));
        return false;
      }
    ...
    case BoneOriginReferenceMode::kHierarchyDeform: {
      if (!hierarchyDeformBoneOrigins(ctx.ftl, ctx.reference_ftl, options.hierarchy_deform_step_limit)) return false;
      break;
    }
  }

  if (options.snap_action_points) {
    ArxReturnCode rc = pistoris::snapFtlActionPointsToReference(ctx.ftl, ctx.reference_ftl);
    if (rc != ARX_OK) {
      cli::diagnostic(cli::DiagnosticCode::kModelModuleFailed,
                      "--snap-action-points-to-reference failed: %s (code %d)",
                      pistoris::errorString(rc),
                      static_cast<int>(rc));
      return false;
    }
  ...
  if (options.copy_reference_affiliations) {
    ArxReturnCode rc = pistoris::copyFtlSyntheticSelectionAffiliations(ctx.ftl, ctx.reference_ftl);
    if (rc != ARX_OK) {
      cli::diagnostic(cli::DiagnosticCode::kModelModuleFailed,
                      "--copy-synthetic-selection-affiliations failed: %s (code %d)",
                      pistoris::errorString(rc),
                      static_cast<int>(rc));
      return false;
    }
